...
def parameter_output(params):
    ...
    txt_file_path = os.path.join(output_dir, 'NICoTR-C_Parameters.txt')
    ...
    excel_file_path = os.path.join(output_dir, 'NICoTR-C_Parameters.xlsx')
...
class NICoTRC(nn.Module):
    def __init__(self, Nway, R0, R_Align):
        super(NICoTRC, self).__init__()
        N1, N2, N3 = Nway
        ...
        self.R2_net = nn.Sequential(
            nn.Conv2d(R0_2, RA_2, kernel_size=3, padding=1),
            nn.LeakyReLU(),
        )
        self.R31_net = nn.Sequential(
            nn.Conv2d(R0_3 * R0_1, RA_3 * RA_1, kernel_size=3, padding=1),
            nn.LeakyReLU(),
    ...
    def forward(self):
        ...
        G1=G1.permute(2, 0, 1).unsqueeze(0)
        G1 = self.R2_net(G1)
        G1 =G1.squeeze(0).permute(1, 2, 0)
        ...
        M_rotated = M_rotated.reshape(N1 , N2, R0_1 * R0_3)
        M_rotated = M_rotated.permute(2, 0, 1).unsqueeze(0)
        M_rotated = self.R31_net(M_rotated)
        M_rotated = M_rotated.squeeze(0).permute(1, 2, 0)
        M_rotated = M_rotated.reshape(N1*N2, RA_1 * RA_3)
        ...
        for R_1 in [15]:
            for R_2 in [25]:
                for R_3 in [12]:
                    ...
                    model = NICoTRC(Nway, R_0, R_Align).type(dtype)
                    params = []
                    params += [x for x in model.parameters()]

                    s = sum([np.prod(list(p.size())) for p in params]);
                    print('Number of params: %d' % s)
                    optimizier = optim.Adam(params, lr=0.004, weight_decay=1e-7 )
                    ...
                    # filename = f"result_MSI/{data}_{SR}_IrTRC.mat"
